...
class Stork
   ...
   def predeploy_sign(file_paths, package_config)
      ...
      SIGNING["password"] = prompt("Code signing pfx password?") unless SIGNING["password"]
      sign_password = SIGNING["password"];

      file_paths.each do |file_path|
         sign_result = `#{signtool_path} sign /f #{pfx_path} /t #{timestamp_url} /p #{sign_password} #{egg_path}/#{file_path}`;
      end
   end
end
